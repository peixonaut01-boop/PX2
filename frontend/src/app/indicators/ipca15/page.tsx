...
// Helper: format date for display (e.g., "Novembro de 2025")
function formatDateLong(isoDate: string): string {
  const date = parseDate(isoDate);
  return date.toLocaleDateString("pt-BR", { month: "long", year: "numeric" });
}

// Helper: format date short (e.g., "nov/25")
...
function prepareChartData(dataKey: "mom" | "a12", startYear: number = 2020) {
  const rawData = ipca15Data[dataKey] as Record<string, unknown>[];
  const startDate = parseDate(`${startYear}-01-01`);

  const filtered = rawData.filter((row) => {
    const rowDate = parseDate(row.data_date as string);
    if (dataKey === "a12") {
      return rowDate >= startDate && row.IPCA15 !== null;
    }
...
export default function Ipca15Page() {
  const momData = ipca15Data.mom as Record<string, unknown>[];
  const a12Data = ipca15Data.a12 as Record<string, unknown>[];
  const pesosData = ipca15Data.pesos as Record<string, unknown>[];
  ...
  // Calculate deltas
  const ipcaMomDelta = (latestMom.IPCA15 as number) - (previousMom.IPCA15 as number);
  ...
  const ipcaYoY = sameMonthLastYearA12
    ? (latestA12.IPCA15 as number) - (sameMonthLastYearA12.IPCA15 as number)
    : null;
  ...
  // Diffusion data
  const difusaoData = (ipca15Data.difusao as Record<string, unknown>[] | undefined)
    ?.filter((row) => {
      const rowDate = parseDate(row.data_date as string);
  ...
  // Table indicators
  const keyIndicators = [
    { key: "IPCA15", label: "IPCA-15 (Índice Geral)" },
  ...
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="mx-auto max-w-7xl px-4 py-8">
        {/* Breadcrumb */}
        <div className="mb-4 text-sm text-gray-700 dark:text-slate-200">
          <Link href="/" className="text-blue-600 hover:underline dark:text-blue-300">
            Início
          </Link>{" "}
          /{" "}
          <Link href="/indicators" className="text-blue-600 hover:underline dark:text-blue-300">
            Brasil
          </Link>{" "}
          / <span className="text-gray-700 dark:text-slate-200 font-medium">IPCA-15</span>
        </div>

        {/* Header */}
        <section className="mb-8 rounded-3xl px-8 py-8 text-slate-900 dark:text-white shadow-xl relative overflow-hidden border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <div className="relative z-10">
            <div className="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-200 mb-2 font-medium">
              <span>Macro Brasil</span>
              <span>•</span>
              <span>Inflação</span>
            </div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">
              IPCA-15 de <span className="capitalize">{formattedDate}</span>
            </h1>
            <p className="mt-3 max-w-2xl text-sm text-slate-800 dark:text-slate-100">
              Prévia do IPCA, coletada entre o dia 16 do mês anterior e o dia 15 do mês de
              referência.
            </p>
            <p className="mt-2 text-xs text-slate-700 dark:text-slate-200">
              Atualizado em {formatUpdateDate(ipca15Data.metadata.last_updated)}
            </p>
          </div>
        </section>

        {/* Sumário Executivo - Texto Corrido */}
        <section className="mb-8 rounded-2xl border-0 dark:border dark:border-blue-400 border-l-4 border-l-blue-600 dark:border-l-blue-400 bg-white dark:bg-slate-900 p-6 shadow-sm text-gray-900 dark:text-slate-200">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-blue-600 mb-3 dark:text-blue-300">Sumário Executivo</h2>
          <p className="leading-relaxed text-justify">
            O <strong>IPCA-15</strong> de <span className="capitalize">{formattedDate}</span>{" "}
            {(latestMom.IPCA15 as number) >= 0 ? "registrou alta" : "apresentou queda"} de{" "}
            <strong className="text-blue-700 dark:text-blue-400">{Math.abs(latestMom.IPCA15 as number).toFixed(2)}%</strong>{" "}
            {ipcaMomDelta !== 0 && (
              <>({ipcaMomDelta > 0 ? "aceleração" : "desaceleração"} de {Math.abs(ipcaMomDelta).toFixed(2)} p.p. frente aos {(previousMom.IPCA15 as number).toFixed(2)}% de {previousMonthDate})</>
            )}, acumulando{" "}
            <strong className="text-blue-700 dark:text-blue-400">{(latestA12.IPCA15 as number).toFixed(2)}%</strong> em 12 meses
            {ipcaYoY !== null && (
        ...
        <section className="mb-6 rounded-2xl bg-white dark:bg-slate-900/70 border border-gray-200 dark:border-slate-800 p-4">
          <div className="grid md:grid-cols-3 gap-4">
            <InflationChart
              data={chartDataA12}
              series={mainSeries}
              title="IPCA-15, Livres e Administrados — A12 (%)"
            />
            <InflationChart
              data={chartDataA12}
        ...
        <section className="mb-8 rounded-2xl bg-white dark:bg-slate-900/70 border border-gray-200 dark:border-slate-800 p-4">
          <div className="grid md:grid-cols-3 gap-4">
            <InflationChart
              data={chartDataA12}
        ...
        {/* Footer Note */}
        <section className="mt-8 p-4 bg-gray-100 dark:bg-slate-900/70 rounded-xl text-sm text-slate-800 dark:text-slate-100 border border-gray-200 dark:border-slate-800">
          <strong className="font-semibold">Fonte:</strong> IBGE/Sidra. Dados processados automaticamente.
          <br />
          <strong className="font-semibold">Metodologia:</strong> Núcleos calculados com ponderação pelos pesos do IPCA-15.
        </section>
      </div>
    </main>
  );
